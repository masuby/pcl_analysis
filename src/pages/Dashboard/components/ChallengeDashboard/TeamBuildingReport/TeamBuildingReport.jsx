...
import { buildTeamBuildingEmailHTML } from './utils/teamBuildingEmailTemplate';
import { processTeamBuildingReport } from './utils/teamBuildingProcessor';
import {
  downloadTeamBuildingReport,
  buildTeamBuildingReportBuffer,
  bufferToBase64,
  fmtSize,
} from './utils/teamBuildingExport';
...
import './TeamBuildingReport.css';

const TBR_RECIPIENTS_KEY = 'tbr_email_recipients';
...
function EmailModal({ processedData, onClose }) {
  ...
  const [recipients, setRecipients] = useState(() => {
    try { return JSON.parse(localStorage.getItem(TBR_RECIPIENTS_KEY) || '[]'); } catch { return []; }
  });
  const [newEmail,  setNewEmail]  = useState('');
  const [pasteBox,  setPasteBox]  = useState('');
  const [subject,   setSubject]   = useState(
    `TEAM BUILDING QUALIFICATION REPORT — ALL STAFF — ${monthsInData.join(' · ')}`
  );
  ...
  useEffect(() => {
    try { localStorage.setItem(TBR_RECIPIENTS_KEY, JSON.stringify(recipients)); } catch {}
  }, [recipients]);
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      const { buffer, fileName } = buildTeamBuildingReportBuffer(processedData);
      const base64   = bufferToBase64(buffer);
      const htmlBody = buildTeamBuildingEmailHTML(summary, monthsInData);
      const attachments = [{ base64, name: fileName }];
      const criteria = await fetchCriteriaAttachment('TEAM_BUILDING_CRITERIA');
  ...
  return (
    <div className="tbr-modal-overlay" onClick={(e) => { if (e.target === e.currentTarget && !sending) onClose(); }}>
      <div className="tbr-modal tbr-modal--email">

        {/* header */}
        <div className="tbr-modal-header">
          <h2 className="tbr-modal-title">✉️ Send Report by Email</h2>
          <button className="tbr-modal-close" onClick={onClose} disabled={sending}>×</button>
        </div>

        {/* progress banner */}
        {progress && (
          <div className={`tbr-progress-banner ${allSuccess ? 'tbr-progress-banner--ok' : ''}`}>
            <div className="tbr-progress-title">
              {sending ? 'Sending…' : allSuccess ? '✓ Sent successfully' : 'Send result'}
            </div>
            <ul className="tbr-progress-list">
              {progress.map(({ email, status, error }) => (
                <li key={email} className={`tbr-progress-item tbr-progress-item--${status}`}>
                  <span className="tbr-progress-email">{email}</span>
                  <span className="tbr-progress-icon">
                    {status === 'sending' && '⏳'}
                    {status === 'success' && '✓'}
                    {status === 'failed'  && '✗'}
                  </span>
                  {status === 'failed' && error && <span className="tbr-progress-err">{error}</span>}
                </li>
              ))}
            </ul>
            {allDone && (
              <button className="tbr-btn tbr-btn--cancel" style={{ marginTop: 10 }} onClick={onClose}>
                Close
              </button>
            )}
          </div>
        ...
        {!progress && (
          <div className="tbr-modal-body">
            ...
            {/* subject */}
            <p className="tbr-modal-label">Subject</p>
            <input
              className="tbr-modal-input"
              type="text"
              value={subject}
            ...
            {/* add recipient */}
            <p className="tbr-modal-label">
              Recipients <span className="tbr-modal-hint">(saved for next time)</span>
            </p>
            <div className="tbr-recipient-row">
              <input
                ref={inputRef}
                className="tbr-modal-input"
                type="email"
              ...
              />
              <button className="tbr-btn tbr-btn--upload" onClick={addEmail}>Add</button>
              <button className="tbr-btn tbr-btn--replace" onClick={pasteFromClipboard} title="Paste from clipboard">
                📋 Paste
              </button>
            </div>
            ...
            {/* chips */}
            {recipients.length > 0 && (
              <div className="tbr-recipient-chips">
                {recipients.map((e) => (
                  <span key={e} className="tbr-recipient-chip">
                    {e}
                    <button className="tbr-chip-remove" onClick={() => removeEmail(e)}>×</button>
                  </span>
                ))}
              </div>
            )}
            ...
            {/* paste box */}
            <details className="tbr-paste-details">
              <summary className="tbr-paste-summary">Paste multiple emails at once</summary>
              <textarea
                className="tbr-modal-textarea"
                rows={3}
                placeholder="One per line, or comma / semicolon separated"
                value={pasteBox}
                onChange={(e) => setPasteBox(e.target.value)}
              />
              <button className="tbr-btn tbr-btn--replace" onClick={addPasted}>Add pasted</button>
            </details>
            ...
            {err && <div className="tbr-error">{err}</div>}

            {/* actions */}
            <div className="tbr-modal-actions">
              <button className="tbr-btn tbr-btn--cancel" onClick={onClose}>Cancel</button>
              <button
                className="tbr-btn tbr-btn--email tbr-btn--lg"
                onClick={handleSend}
...
function UploadRow({ typeDef, fileRecord, onUploaded, onDeleted, showToast }) {
  ...
  return (
    <div className="tbr-upload-row">
      <div className={`tbr-dot ${hasFile ? 'tbr-dot--ready' : 'tbr-dot--missing'}`} />
      <div className="tbr-upload-icon">{typeDef.icon}</div>
      <div className="tbr-upload-info">
        <div className="tbr-upload-title">{typeDef.label}</div>
        {hasFile ? (
          <>
            <div className="tbr-upload-filename" title={fileRecord.fileName}>{fileRecord.fileName}</div>
            <div className="tbr-upload-meta">
              {fmtSize(fileRecord.fileSize)} · {new Date(fileRecord.createdAt).toLocaleDateString('en-GB')}
            </div>
          </>
        ) : (
          <div className="tbr-upload-desc">{typeDef.desc}</div>
        )}
        {err && <div className="tbr-error">{err}</div>}
      </div>
      ...
      <div className="tbr-btn-group">
        {typeDef.kind === 'SALES' && (
          <button
            className="tbr-btn tbr-btn--refresh"
        ...
        {hasFile ? (
          <>
            <button className="tbr-btn tbr-btn--replace" onClick={() => inputRef.current?.click()} disabled={uploading || deleting || downloading || refreshing}>
              {uploading ? 'Uploading…' : '↩ Replace'}
            </button>
            <button className="tbr-btn tbr-btn--download" onClick={handleDownload} disabled={uploading || deleting || downloading || refreshing} title="Download this file">
              {downloading ? '…' : '⬇'}
            </button>
            <button className="tbr-btn tbr-btn--delete" onClick={() => setConfirmOpen(true)} disabled={uploading || deleting || downloading || refreshing}>
              {deleting ? '…' : '🗑'}
            </button>
          </>
        ) : (
          <button className="tbr-btn tbr-btn--upload" onClick={() => inputRef.current?.click()} disabled={uploading || refreshing}>
...
function PreviewTable({ hierarchy, monthsInData }) {
  ...
  const ProductCell = ({ product, pObj, rowSpan }) => (
    <td rowSpan={rowSpan} className="tbr-merged-product">
      <div className="tbr-merged-name">
        <span className={`tbr-badge tbr-badge--${product.toLowerCase()}`}>{product}</span>
        {' '}{PRODUCT_LABELS[product] ?? product}
      </div>
      <div className="tbr-merged-meta">
        Tgt: {Math.round(pObj.target / 1_000_000).toLocaleString()}M
        &nbsp;·&nbsp;Act: {Math.round(pObj.totalAmount / 1_000_000).toLocaleString()}M
      </div>
      <div className="tbr-merged-meta" style={{ fontWeight: 700, color: pctColor(pObj.totalAmount, pObj.target) }}>
  ...
  const RegionCell = ({ region, rObj, rowSpan }) => {
    ...
    return (
      <td rowSpan={rowSpan} className="tbr-merged-region">
        <div className="tbr-merged-name">{region}</div>
        <div className="tbr-merged-meta">
          Tgt: {Math.round(rObj.target / 1_000_000).toLocaleString()}M
          &nbsp;·&nbsp;Act: {Math.round(rObj.totalAmount / 1_000_000).toLocaleString()}M
        </div>
        <div className="tbr-merged-meta">{rObj.totalLoans.toLocaleString()} loans</div>
        <div className="tbr-merged-meta">
        ...
        <div className={`tbr-qual-badge ${rObj.regionQualified ? 'tbr-qual--yes' : 'tbr-qual--no'}`}>
          {rObj.regionQualified ? '✓ QUALIFIED' : '✗ NOT QUALIFIED'}
        </div>
        {!rObj.regionQualified && rObj.regionReason && (
          <div className="tbr-qual-reason">{rObj.regionReason}</div>
        )}
      </td>
    );
  };
  ...
  const BranchCell = ({ branch, bObj, rowSpan }) => {
    ...
    return (
      <td rowSpan={rowSpan} className="tbr-merged-branch">
        <div className="tbr-merged-name">{branch}</div>
        {bObj.tlName && <div className="tbr-merged-meta" style={{ fontStyle: 'italic' }}>TL: {bObj.tlName}</div>}
        <div className="tbr-merged-meta">
        ...
        <div className={`tbr-qual-badge ${bObj.tlQualified ? 'tbr-qual--yes' : 'tbr-qual--no'}`}>
          {bObj.tlQualified ? '✓ QUALIFIED' : '✗ NOT QUAL.'}
        </div>
        {!bObj.tlQualified && bObj.tlReason && (
          <div className="tbr-qual-reason">{bObj.tlReason}</div>
        )}
      </td>
    );
  };
  ...
  return (
    <div className="tbr-table-wrap">
      <table className="tbr-table">
        ...
        <tbody>
          {flatRows.map((row, i) => (
            <tr key={i} className="tbr-row--agent">
              ...
              <td style={{ textAlign: 'right', color: 'var(--tbr-text-muted,#9ca3af)' }}>{row.idx + 1}</td>
              <td style={{ fontWeight: 600 }}>{row.agent.repName}</td>
              <td style={{ color: 'var(--tbr-text-muted,#6b7280)', fontSize: '0.74rem' }}
                  title={row.agent.role || ''}>
                {row.agent.title || row.agent.role || '—'}
              </td>
              <td>
                <span className={`tbr-badge tbr-badge--${row.agent.flag === 'Yes' ? 'old' : 'new'}`}>
                  {row.agent.flag === 'Yes' ? 'Old' : 'New'}
                </span>
              </td>
              <td style={{ fontSize: '0.74rem' }}>{row.agent.period || 'Unknown'}</td>
              <td>
                <span className={`tbr-badge tbr-badge--${row.agent.flag === 'Yes' ? 'yes' : 'no'}`}>
                  {row.agent.flag}
                </span>
              </td>
              ...
              {months.map((m) => {
                ...
                return (
                  <td key={m} style={{ textAlign: 'right', color: mo.amt > 0 ? 'inherit' : 'var(--tbr-text-muted,#9ca3af)' }}>
                    {mo.amt > 0
              ...
              <td style={{ textAlign: 'right', color: 'var(--tbr-text-muted,#9ca3af)' }}>
              ...
              <td style={{ textAlign: 'center' }}>
                <span className={`tbr-badge ${row.agent.qualified ? 'tbr-badge--yes' : 'tbr-badge--no'}`}>
                  {row.agent.qualified ? 'QUALIFIED' : 'NOT QUALIFIED'}
                </span>
                {!row.agent.qualified && row.agent.qualReason && (
                  <div className="tbr-qual-reason">{row.agent.qualReason}</div>
                )}
              </td>
            </tr>
          ))}
...
function SummaryStats({ summary }) {
  ...
  return (
    <div className="tbr-stats-row">
      <div className="tbr-stat">
        <div className="tbr-stat-value">{totalAgents.toLocaleString()}</div>
        <div className="tbr-stat-label">Total Agents</div>
      </div>
      <div className="tbr-stat tbr-stat--green">
        <div className="tbr-stat-value">{(qualified ?? 0).toLocaleString()}</div>
        <div className="tbr-stat-label">Qualified Reps</div>
      </div>
      <div className="tbr-stat" style={{ '--stat-color': '#dc2626' }}>
        <div className="tbr-stat-value" style={{ color: '#dc2626' }}>{(notQualified ?? 0).toLocaleString()}</div>
        <div className="tbr-stat-label">Not Qualified</div>
      </div>
      <div className="tbr-stat tbr-stat--green">
        <div className="tbr-stat-value">{(qualifiedTLs ?? 0).toLocaleString()}</div>
        <div className="tbr-stat-label">Qualified TLs</div>
      </div>
      <div className="tbr-stat tbr-stat--green">
        <div className="tbr-stat-value">{(qualifiedRegions ?? 0).toLocaleString()}</div>
        <div className="tbr-stat-label">Qualified Regions</div>
      </div>
      <div className="tbr-stat tbr-stat--blue">
        <div className="tbr-stat-value">{totalLoans.toLocaleString()}</div>
        <div className="tbr-stat-label">Total Loans</div>
      </div>
      <div className="tbr-stat tbr-stat--purple">
        <div className="tbr-stat-value">{Math.round(totalAmount / 1_000_000).toLocaleString()}M</div>
        <div className="tbr-stat-label">Disbursed (TZS)</div>
      </div>
      <div className="tbr-stat">
        <div className="tbr-stat-value">{oldAgents}</div>
        <div className="tbr-stat-label">Old Agents</div>
      </div>
      <div className="tbr-stat">
        <div className="tbr-stat-value">{newAgents}</div>
        <div className="tbr-stat-label">New Agents</div>
      </div>
      {Object.entries(byProduct).map(([p, b]) => (
        <div key={p} className="tbr-stat">
          <div className="tbr-stat-value">
            {Math.round(b.totalAmount / 1_000_000).toLocaleString()}M
            {b.target > 0 && (
              <span style={{ fontSize: '0.85rem', fontWeight: 400, color: 'var(--tbr-text-muted,#6b7280)' }}>
                {' '}/ {Math.round(b.target / 1_000_000)}M
              </span>
            )}
          </div>
          <div className="tbr-stat-label">{p} · {b.qualified ?? 0} qualified</div>
        </div>
      ))}
    </div>
  );
...
const TeamBuildingReport = () => {
  ...
  const handleDownload = useCallback(() => {
    if (processedData) downloadTeamBuildingReport(processedData);
  }, [processedData]);
  ...
  return (
    <div className="tbr-root">
      ...
      <div className="tbr-header">
        <h1 className="tbr-title">Team Building Report</h1>
        <div className="tbr-audience">ALL STAFF</div>
      ...
      <CriteriaFileManager
        reportType="TEAM_BUILDING_CRITERIA"
        title="Team Building Criteria — All Staff"
      />

      {/* ── file uploads ── */}
      <div className="tbr-section">
        <div className="tbr-section-label">Source Files</div>
        {loading ? (
          <div className="tbr-spinner-wrap">
            <div className="tbr-spinner" />
            <span>Loading…</span>
          </div>
        ) : (
          <div className="tbr-upload-list">
            {FILE_TYPES.map((t) => (
              <UploadRow
                key={t.kind}
                typeDef={t}
      ...
      {!loading && (
        <div className="tbr-section">
          <div className="tbr-action-bar">
            <span className={`tbr-status ${statusClass}`}>{statusLabel}</span>

            <button
              className="tbr-btn tbr-btn--gen tbr-btn--lg"
            ...
            {processedData && (
              <>
                <button className="tbr-btn tbr-btn--dl tbr-btn--lg" onClick={handleDownload}>
                  ⬇ Download Excel
                </button>
                <button className="tbr-btn tbr-btn--email tbr-btn--lg" onClick={() => setShowEmail(true)}>
                  ✉️ Send Email
                </button>
              </>
            )}

            <span className="tbr-action-note">
              {allReady ? `Months covered once generated` : `${FILE_TYPES.length - readyCount} file(s) still needed`}
            </span>
          </div>

          {procErr && <div className="tbr-error" style={{ marginTop: 10 }}>⚠ {procErr}</div>}
        </div>
      )}
      ...
      {processedData && (
        <div className="tbr-section">
          <div className="tbr-section-label">
...
export default TeamBuildingReport;
