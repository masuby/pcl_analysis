...
function EmailModal({ processedData, onClose }) {
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      const criteria = await fetchCriteriaAttachment('LOCAL_TRIP_CRITERIA');
  ...
  return (
    <div className="ltr-upload-row">
      ...
      <div className="ltr-btn-group">
        {typeDef.kind === 'SALES' && (
          <button
            className="ltr-btn ltr-btn--refresh"
        ...
        {hasFile ? (
          <>
            <button className="ltr-btn ltr-btn--replace" onClick={() => inputRef.current?.click()} disabled={uploading || deleting || downloading || refreshing}>
              {uploading ? 'Uploading…' : '↩ Replace'}
            </button>
            <button className="ltr-btn ltr-btn--download" onClick={handleDownload} disabled={uploading || deleting || downloading || refreshing} title="Download this file">
              {downloading ? '…' : '⬇'}
            </button>
            <button className="ltr-btn ltr-btn--delete" onClick={() => setConfirmOpen(true)} disabled={uploading || deleting || downloading || refreshing}>
              {deleting ? '…' : '🗑'}
            </button>
          </>
        ) : (
          <button className="ltr-btn ltr-btn--upload" onClick={() => inputRef.current?.click()} disabled={uploading || refreshing}>
