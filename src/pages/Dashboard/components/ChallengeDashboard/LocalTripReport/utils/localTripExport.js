/**
 * Local Trip Report — Excel Export
 *
...
function buildAllAgentsSheet(hierarchy, monthsInData) {
  ...
  sortedProducts.forEach((product) => {
    ...
    Object.entries(pObj.regions)
      ...
      Object.entries(rObj.branches)
        ...
        sortedAgents.forEach((agent, idx) => {
          ...
          // ROLE col now shows the precise TITLE from the Users file (more accurate than Role)
        ...
        const branchLabel = [
          ...
          bObj.tlTitle ? `(${bObj.tlTitle})`    : '',
          `Tgt: ${fmt(bObj.target)} | Act: ${fmt(bObj.totalAmount)}`,
          `Sales: ${tlPct}%  New Biz: ${nbPct}%  Loans: ${lnPct}%  / req ${reqPct}%`,
          `NewBiz Tgt: ${fmt(bObj.tlNewBizTarget ?? 0)} | Loans Tgt: ${bObj.loansTarget ?? 0}`,
          `PAR>30: ${((bObj.tlPar30 ?? 0) * 100).toFixed(1)}%`,
      ...
      const regNBPct = Math.round((rObj.regionNewBizAchv ?? 0) * 100);
      const regLnPct = Math.round((rObj.regionLoansAchv  ?? 0) * 100);
      const regReqPct = Math.round((rObj.regionRequiredRatio ?? 1.3) * 100);
      const regionLabel = [
        region.toUpperCase(),
        `Tgt: ${fmt(rObj.target)} | Act: ${fmt(rObj.totalAmount)}`,
        `Sales: ${regPct}%  New Biz: ${regNBPct}%  Loans: ${regLnPct}%  / req ${regReqPct}%`,
        `NewBiz Tgt: ${fmt(rObj.regionNewBizTarget ?? 0)} | Loans Tgt: ${rObj.loansTarget ?? 0}`,
        `PAR>30: ${((rObj.regionPar30 ?? 0) * 100).toFixed(1)}%`,
    ...
    const nbPctP       = pObj.newBizTarget > 0 ? Math.round((pObj.newBizAmount / pObj.newBizTarget) * 100) : 0;
    const lnPctP       = pObj.loansTarget  > 0 ? Math.round((pObj.totalLoans   / pObj.loansTarget)  * 100) : 0;
    const productLabel = [
      pName,
      `Target: ${fmt(pObj.target)} TZS`,
      `Actual: ${fmt(pObj.totalAmount)} TZS`,
      `Sales: ${pct}%  New Biz: ${nbPctP}%  Loans: ${lnPctP}%`,
      `Loans: ${pObj.totalLoans} of ${pObj.loansTarget ?? 0} target`,
...
function buildNotQualifiedSheet(hierarchy, monthsInData) {
  ...
  // Sort: cascade-fail rows (agent met criteria, TL didn't) first by % achv DESC,
  // then everyone else by % achv DESC.
  allNotQual.sort((x, y) => {
    const xCascade = x.agent.selfQualified === true ? 1 : 0;
    const yCascade = y.agent.selfQualified === true ? 1 : 0;
    if (xCascade !== yCascade) return yCascade - xCascade;  // cascade group first
    return cmpAchv(x.agent.totalAmount, x.agent.target, y.agent.totalAmount, y.agent.target);
  });

  allNotQual.forEach(({ pName, region, branch, agent }, idx) => {
    const alt = idx % 2 === 1;
    // Cascade-fail rows (agent passed, TL didn't) get an amber tint so they
    // stand out — these are the agents who "would have qualified".
    const cascadeFail = agent.selfQualified === true && agent.tlQualified === false;
    const rowBg       = cascadeFail
      ? (alt ? 'FEF3C7' : 'FFFBEB')   // amber tint
      : null;
    ...
    // Override row background colour for cascade-fail rows (overrides alt-banding)
    if (rowBg) {
      [1, 2, 3, 4, 5, 6, 7, 8].forEach((c) => {
        if (row[c]) row[c] = { ...row[c], s: { ...row[c].s, fill: FILL(rowBg) } };
      });
    }

    row[RIGHT_S + 4] = {
      v: cascadeFail ? `⚠ Agent met criteria but TL did not qualify` : (agent.qualReason || '—'),
      t: 's',
      s: {
        font:      F(cascadeFail, cascadeFail ? '92400E' : '9C0006', 9),
        fill:      FILL(cascadeFail ? 'FEF3C7' : (alt ? 'FEE2E2' : 'FFF5F5')),
...
function buildSummarySheet(summary, monthsInData) {
  ...
  const rows = [
    [mk('LOCAL TRIP QUALIFICATION REPORT — SALES DEPARTMENTS', true, HDR, HDR_FG, 'left', 12), mk('',false,HDR), mk('',false,HDR), mk('',false,HDR), mk('',false,HDR), mk('',false,HDR)],
...
function buildCriteriaSheet() {
  ...
  // ── Title ───────────────────────────────────────────────────────────────────
  rows.push(fullRow('LOCAL TRIP QUALIFICATION CRITERIA — 2026', true, HDR, HDR_FG, 13));
...
export function downloadLocalTripReport(processedData) {
  ...
  XLSXStyle.writeFile(wb, `Local_Trip_Report_${new Date().toISOString().slice(0,10)}.xlsx`);
}

export function buildLocalTripReportBuffer(processedData) {
  const { hierarchy, monthsInData, summary } = processedData;
  const wb = XLSXStyle.utils.book_new();
  XLSXStyle.utils.book_append_sheet(wb, buildSummarySheet(summary, monthsInData),        'Summary');
  XLSXStyle.utils.book_append_sheet(wb, buildAllAgentsSheet(hierarchy, monthsInData),    'All Agents');
  ...
  const fileName = `Local_Trip_Report_${date}.xlsx`;
