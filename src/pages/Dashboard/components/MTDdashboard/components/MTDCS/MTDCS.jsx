...
const MTDCS = () => {
  ...
  const { reports, parsedData, loading, error, hasData } = useMTDData('CS', selectedDate);
  ...
  if (!hasData) {
    return (
      <div className="mtd-empty-wrap">
        ...
        <h2>No CS MTD Reports Found</h2>
        <p className="mtd-empty-sub">
          Upload CS MTD reports in the Administration page to see analysis.
        </p>
      </div>
    );
  }
      ...
      {parsedData && (
        <div className="mtd-date-header">
          ...
          <span className="mtd-date-text">
            CS MTD - {parsedData.reportDate instanceof Date
