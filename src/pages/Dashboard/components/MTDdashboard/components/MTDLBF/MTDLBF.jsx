...
const MTDLBF = () => {
  ...
  const { reports, parsedData, loading, error, hasData } = useMTDData('LBF', selectedDate);
  ...
  if (!hasData) {
    return (
      <div className="mtd-empty-wrap">
        ...
        <h2>No LBF MTD Reports Found</h2>
        <p className="mtd-empty-sub">
          Upload LBF MTD reports in the Administration page to see analysis.
        </p>
      </div>
    );
  }
