...
export default function RepeatBusinessPerformance({ comparisonData, trendData, logoSrc }) {
  ...
  const explanation = `The total amount disbursed for repeat business for the month of ${monthLabel} has ${lmText} in comparison to ${lastMonthLabel || 'the previous month'}, and ${lyText} in comparison to ${lastYearLabel || 'the same month last year'}.`;

  return (
    <div className="report-page">
      <div className="report-header">
        <h2 className="report-title">REPEAT BUSINESS SALES PERFORMANCE</h2>
          ...
          <div style={{ width: '100%', height: '320px', marginTop: '1rem' }}>
            <ResponsiveContainer>
              <LineChart data={trendData} margin={{ top: 30, right: 30, left: 60, bottom: 30 }}>
                ...
                <Tooltip
                  formatter={(val) => [(val / 1e9).toFixed(2) + ' Billion TZS', 'Repeat Business']}
                  contentStyle={{ fontSize: 12, background: '#fff', border: '1px solid #e2e8f0' }}
                />
                <Line
                  type="monotone"
                  dataKey="repeatBusiness"
                  stroke={PRIMARY_BLUE}
                  strokeWidth={3}
