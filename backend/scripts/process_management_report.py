...
Input folder: Management Report, Client file, Zone and cluster file (all in same folder)
- Management Report* (e.g. Management Report2026-02.xlsx)
- Client* (e.g. Clientstz.xlsx)
- Zone and cluster* (e.g. Zone and cluster.xlsx)
...
Usage:
  python backend/scripts/process_management_report.py
  python backend/scripts/process_management_report.py "C:/path/to/Management"

Or set env: MANAGEMENT_FOLDER=C:/path/to/Management
"""
...
DEFAULT_WORK_DIR = Path(r"C:\Users\Daniel\Desktop\Management")
...
def main():
    ...
    if "--inspect" in sys.argv:
        ...
        path = Path(args[0]) if args else DEFAULT_WORK_DIR
        if path.is_file():
    ...
    work_dir = Path(
        os.environ.get("MANAGEMENT_FOLDER") or
        (args[0] if args else str(DEFAULT_WORK_DIR))
    ).resolve()

    mgmt_file = find_file(work_dir, "Management Report")
    client_file = find_file(work_dir, "Client")
    zone_file = find_file(work_dir, "Zone and cluster")

    if not mgmt_file:
        print(f"Error: No Management Report file found in {work_dir}")
        sys.exit(1)
    if not client_file:
        print(f"Error: No Client file found in {work_dir}")
        sys.exit(1)
    if not zone_file:
        print(f"Error: No Zone and cluster file found in {work_dir}")
        sys.exit(1)
    ...
    print(f"  Clients:    {client_file.name}")
    print(f"  Zone/Cluster: {zone_file.name}")
    print()
