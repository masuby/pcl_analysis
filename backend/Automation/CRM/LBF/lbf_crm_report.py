...
processed_file = fr"C:\Users\Daniel\Desktop\code\pcl\CRM\LBF\NEW_EXCEL\CRM_PROCESSED_{timestamp}.xlsx"
...
def copy_matching_columns(master_ws, processed_ws, sheet_name):
    ...
    # Clear only data content (not formatting, pivot tables, etc.)
    # Find the last row with data in master sheet
    master_used_range = master_ws.used_range
    if master_used_range:
        last_row = master_used_range.last_cell.row
        # Clear only data rows (keep header and formatting)
        if last_row > 1:
            master_ws.range(f'A2:{master_used_range.last_cell.address}').clear_contents()
    
    # Copy headers to master sheet if they don't exist
    if not master_ws.range('A1').value:
...
def copy_sheets_to_master():
    ...
    try:
        ...
        # Define sheets to copy (only visible data sheets, exclude hidden and pivot sheets)
        sheets_to_copy = [
            'user_data', 
            'activity_data', 
            'agent_activity', 
        ...
        # Filter to only include visible sheets that exist in both workbooks
        visible_sheets_to_copy = []
        for sheet_name in sheets_to_copy:
            if (sheet_name in master_wb.sheet_names and 
                sheet_name in processed_wb.sheet_names and
                master_wb.sheets[sheet_name].visible):
                visible_sheets_to_copy.append(sheet_name)
            else:
                print(f"  Skipping {sheet_name} - not found or not visible")
        
        print("Copying sheets with column matching...")
        
        for sheet_name in visible_sheets_to_copy:
        ...
        new_master_file = fr"C:\Users\Daniel\Desktop\code\pcl\CRM\LBF\NEW_EXCEL\LBF_CRM_{MANUAL_DATE.replace('/', '_')}.xlsx"
...
print(f"NEW MASTER FILE CREATED → LBF_CRM_{MANUAL_DATE.replace('/', '_')}.xlsx")
...
print("Only visible sheets processed - hidden sheets preserved")
print("========================================================")