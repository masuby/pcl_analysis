...
def copy_sheets_to_master():
    ...
    # Suppress dialogs that block a headless (visible=False) open:
    #  - the master has inconsistent pivot caches -> Excel wants to "repair"
    #  - the master has external links -> Excel wants to "update links"
    # Without these, Workbooks.Open raises 0x800A03EC ("Open method ... failed").
    app.display_alerts = False
    try:
        app.api.AskToUpdateLinks = False
        app.api.AutomationSecurity = 3  # msoAutomationSecurityForceDisable (block macros silently)
    except Exception:
        pass

    try:
        # Open workbooks (update_links=0 = don't prompt; corrupt_load=1 = xlRepairFile)
        print("Opening workbooks...")
        try:
            master_wb = app.books.open(master_file, update_links=False, corrupt_load=1)
        except Exception:
            # last resort: extract-data load
            master_wb = app.books.open(master_file, update_links=False, corrupt_load=2)
        processed_wb = app.books.open(processed_file, update_links=False)
